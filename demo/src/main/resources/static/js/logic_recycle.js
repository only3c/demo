...
function setPage(container, count, pageindex,pageSize,name,value) {
	...
	if(name==''&&value==''){
		oc.postRequire("get","logic/recycle/list?pageNumber="+pageindex
		...
		oc.postRequire("post","logic/recycle/search","0",param,function(data){
			console.log(data);
			if(data.code=="0"){
				message=JSON.parse(data.message);
				content=message.content;
...
function superaddition(data){
	for (var i = 0; i < data.length; i++) {
		$(".table tbody").append("<tr id='"+data[i].id+"'><td width='8.82%' style='text-align: left;'><div class='checkbox'><input  type='checkbox' value='' name='test' title='全选/取消' class='check'  id='checkboxTwoInput"
						...
						+ i
						+ 1
						+ "'></label></div><span class='pl5' title='"+data[i].logicid+"'>"
						+ data[i].logicid//编号
						+ "</td><td width='8%'>"
						+ data[i].corp_code 
...
function jumpBianse(){
	...
	$(".table tbody tr").dblclick(function(){
	    id=$(this).attr("id");
	    window.location.href="logic_recycle_detail?id="+id+"";
	})
...
//页面加载时的GET请求
function GET(){
	oc.postRequire("get","logic/recycle/list?pageNumber="+pageNumber
		+"&pageSize="+pageSize+"","","",function(data){
			if(data.code=="0"){
				message=JSON.parse(data.message);
				content=message.content;
...
$("#remove").click(function(){
    ...
    oc.postRequire("post","logic/recycle/delete","1",param,function(data){
    	console.log(data);
    	if(data.code=="0"){
    		$("#frame").html("删除成功");
			frame();
...
$("#return").click(function(){
	...
	oc.postRequire("post","logic/recycle/return","1",param,function(data){
		if(data.code=="0"){
    		$("#frame").html("还原成功");
			frame();
    	}else if(data.code=="-1"){
