...
function superaddition(data){
    for (var i = 0; i < data.length; i++) {
        $(".table tbody").append("<tr id='"+data[i].id+"' date='"+data[i].is_admin+"'><td width='8.82%' style='text-align: left;'><div class='checkbox'><input  type='checkbox' value='' name='test' title='全选/取消' class='check'  id='checkboxTwoInput"
                        + i
                        + 1
...
//页面加载时的GET请求
function GET(){
    oc.postRequire("get","user/list?pageNumber="+pageNumber+"&pageSize="
...
$("#remove").click(function(){
    ...
    for(var i=0,date="";i<tr.length;i++){
        var u=$(tr[i]).attr("date");
        if(i<tr.length-1){
            date+=u+",";
        }else{
             date+=u;
    ...
    oc.postRequire("post","user/list/delete","1",param,function(data){
        console.log(data);
        if(data.code=="0"){
            $("#frame").html("删除成功");
            frame();
        }else if(data.code=="-1"){
            $("#frame").html("删除失败,含有系统管理员");
            frame();
        }
    });
})
